...
# Used to get the deque data structure to store the current temperature values
...
# Creates new class to handle temp readings and calculations on those readings
class TemperatureReadings:
    # function to initialze the deque data structure
    def __init__(self, max_len = 100):
        # creates a deque data structure to hold the temperatures
        self.temp_readings = deque(maxlen=max_len)
    
    # read and add the temperature to the deque
    def add_reading(self, temp):
        self.temp_readings.append(temp)

    # Calculate the average of the 100 temperatures
    def get_average(self):
        sum = sum(self.temp_readings)
        average = sum / len(self.temp_readings)
        return average
    
    # Find the Minimum temperature 
    def get_min(self):
        return min(self.temp_readings)

    # Find the Maximum temperature
    def get_max(self):
        return max(self.temp_readings)

# initialize the TempReadings Class
...
class TemperatureMachine(StateMachine):
    ...
    ## thermostat
    ## Since we have 3 states and 1 button we need to click the button multiple times to change the state
    ##
    cycle = (
        off.to(heat) |
        heat.to(cool) |
    ...
    def on_enter_cool(self):
       ...
       # Update thermostat lights on entering cool state
        ...
        # Displays this message on the powershell window on the computer
        if(DEBUG):
    ...
    def manageMyDisplay(self):
        # counter variables used to control the lcd display 
        counter = 1
        altCounter = 1

        # Add the temperature reading to tempReadings every second
        tempReadings.add_reading(self.getFahrenheit())
        ...
        while not self.endDisplay:
            ...
            ## this display is changing every 5 seconds
            if(altCounter < 6):
                # get avg temp
                #avg_temp = tempReadings.get_average()

                # Display the current temperature (T) and Average temperature in degrees Fahrenheit
                lcd_line_2 = f"T:{self.getFahrenheit():0.1f}F AVG:{tempReadings.get_average():0.1f}F"

                #increase altCounter by 1
                altCounter = altCounter + 1
            elif(altCounter < 11):
                # get the minimum and maximum temperatures
                #min_temp = tempReadings.get_min()
                #max_temp = tempReadings.get_max()

                # Display the minimum and maximum temperature readings in degrees Fahrenheit
                lcd_line_2 = f"MN:{tempReadings.get_min():0.1f}F MX:{tempReadings.get_max():0.1f}F"
